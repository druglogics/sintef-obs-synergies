...
Currently, this analysis is mostly based on the use of the [rbbt](http://mikisvaz.github.io/rbbt/) tool [@Vazquez2010] with its specified workflows and tasks for finding the observed synergies of the SINTEF dataset using various parameters as input.
We provide a **docker image** with rbbt and its workflows in the Zenodo dataset [![DOI](https://zenodo.org/badge/DOI/10.5281/zenodo.4022801.svg)](https://doi.org/10.5281/zenodo.4022801) as well as [documentation guidelines](http://tiny.cc/rbbt-doc) on how to use it.

Also, in the [last section](#bliss-all-combinations-method) we are going to validate the method used in [@Flobak2019] to derive statistically significant synergies and compare it with the results from the rbbt-based analysis.
...
1. Download the two datasets from the zenodo dataset [![DOI](https://zenodo.org/badge/DOI/10.5281/zenodo.4022801.svg)](https://doi.org/10.5281/zenodo.4022801)
...
2. Compute the `bliss` and `hsa` task results from the SINTEF dataset (DIY way)
  ...
  Given an `rbbt.SINTEF.image` (it's included in the Zenodo dataset), and a `SINTEF` workflow at least at commit `947307b`, you can use the [run_cimbinator.sh](https://raw.githubusercontent.com/druglogics/sintef-obs-synergies/master/run_cimbinator.sh) script to produce the data results.
